...
import com.example.fileprocessor.dto.IdType;
//import com.example.fileprocessor.reader.AgentInformation;
...
@Component
@Slf4j
public class FSPParentMappingClient {

    @Autowired
    RestTemplate restTemplate;

    public void callCP(String msisdn){
        ...
        processResponse(cp.getBody(), msisdn);
    }

    private void processResponse(ChannelPartner cp, String msisdn) {
...
//            headers.add("x-channelpartner-type", "CANVASSER");
            ...
            IdType idType = IdType.getValue(cp1.getIdtype());
            if(idType == null) {
                return;
            }
            System.out.println("Using Id type - {}" +idType + "for msisdn - {}" +msisdn);
            cpRequest.setDob(splitDob[2]+splitDob[1]+splitDob[0]);
            cpRequest.setIdNo(cp1.getIdno());
            cpRequest.setGender(gender.toUpperCase());
            cpRequest.setIdType(idType.label);
            cpRequest.setMsisdn(msisdn);
            ...
            cpRequest.setGradeCode("FSPDEALER");
            cpRequest.setExtTrId(UUID.randomUUID().toString());
            cpRequest.setDivision("DIVISION");
            cpRequest.setCategoryCode("WHS");
            cpRequest.setRegTypeId("COM_FSPDEALER");

            HttpEntity<Map<String, String>> entity = getMapHttpEntity(cpRequest, headers);
            ...
            if(updateResp.getBody().getStatus().equalsIgnoreCase("200")) {
                System.out.println("Updated Wallet Information for msisdn - {}" + msisdn);
            }else {
                System.out.println("MQ Error while updating msisdn - {}" + msisdn + "with error - {}" + updateResp);
            }

        }else {
            System.out.println("No record found for msisdn - {}" + msisdn);
        }
    }

    private static HttpEntity<Map<String, String>> getMapHttpEntity(ChannelPartnerUpdateRequest cpRequest, HttpHeaders headers) {
        Map<String, String> body = new HashMap<>();
